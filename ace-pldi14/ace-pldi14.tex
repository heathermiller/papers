...
\end{codelisting}
Lines 3-4 introduce \verb|map|, an Ace function governed by the active base \verb|clx.base| taking three arguments. The \emph{active target}, \verb|clx.opencl|, determines which language the function will target (here, the OpenCL kernel language) and mediates code generation.

The body of this function, highlighted in grey for emphasis, does not have Python's semantics. Instead, it will be governed by the active base together with the active types used within it. No such types have been provided explicitly for the arguments yet, however. Because our type system is extensible, the code inside could be meaningful for many different assignments of types to the arguments, so \verb|map| is not yet a \emph{concrete function}. We call functions awaiting types \emph{generic functions}. %Once types have been assigned, they are called \emph{concrete functions}.
...
To produce an output file from an Ace compilation script like \verb|listing|\texttt{\ref{compscript}}\verb|.py|, the command \verb|acec| can be invoked from the shell, as shown in Listing \ref{mapc}. The \verb|acec| compiler operates in two steps:
\begin{enumerate}
\item Executes the provided Python file (\verb|listing3.py|).
\item Extracts the translation from concrete functions and other top-level constructs (e.g. types requiring declarations, or generated imports and pragmas) in the top-level Python environment.  This may produce one or more files (here, just \verb|listing3.cl|) as determined by the active targets that were used (for example, a web framework built upon Ace might produce separate HTML, CSS and JavaScript files; see Sec. \ref{atargets}).
\end{enumerate}

In this case, step 1 prints the output on lines \ref{mapc}.2-\ref{mapc}.4. The type error printed on lines \ref{mapc}.3-\ref{mapc}.4 will be explained in the next section. The compiler (which is a simple Python script) concludes with the message on line \ref{mapc}.5 to indicate that one file was generated. This file is shown in Listing \ref{mapout} and can be used by any programs that consume OpenCL code (e.g. a C file that invokes the generated kernels via the OpenCL host API).
...
\label{mapout}
\end{codelisting}
Lines \ref{compscript}.3-\ref{compscript}.5 construct the types assigned on lines \ref{compscript}.7-\ref{compscript}.10. In Ace, types are themselves values that can be manipulated at compile-time, in contrast to most  contemporary languages, where user-defined types are declaratively written at compile-time but cannot be constructed, inspected or passed around programmatically. More specifically, types are instances of a Python class that implements the \verb|ace.ActiveType| interface (see Sec. \ref{atypes}). Python, then, serves as the \emph{type-level language} of Ace.
As Python values, types can be assigned to variables when desired (removing the need for  facilities like \verb|typedef| in C or \verb|type| in Haskell). Types, like all compile-time objects derived from Ace base classes, do not have visible state and operate in a referentially transparent manner (by constructor memoization, which we do not detail here).% These types are all implemented in the \verb|clx| library imported on line 1, none are built into Ace itself.

The type named \verb|T1| on line \ref{compscript}.3 corresponds to the OpenCL type \verb|global float*|: a pointer to a 32-bit floating point number stored in the compute device's global memory (one of four memory spaces defined by OpenCL \cite{opencl11}). It is constructed by applying \verb|clx.Ptr|, which is an Ace type constructor corresponding to pointer types, to a value representing an  address space, \verb|clx.global_|, and the type being pointed to. That type, \verb|clx.float|, is in turn the Ace type corresponding to \verb|float| in OpenCL (which, unlike C99, is always 32 bits).
The \verb|clx| library contains a full implementation of the OpenCL type system.
We note that we are being more verbose than necessary for the sake of pedagogy. The \verb|clx| library includes a concise shorthand for OpenCL's built-in types: \verb|T1| could be written \verb|clx.gp(clx.f32)|. We also note that Ace is \emph{unopinionated} about issues like memory safety. We will show how to implement safer alternatives to raw pointers in Sec. \ref{examples}\todo{true?}, but we allow users to choose a low level of abstraction if the need arises.

The type \verb|T2| on line \ref{compscript}.4 does not correspond to type in OpenCL directly. It is a pointer to a \emph{complex integer} in global memory. OpenCL does not include primitive support for complex numbers, however -- it is an extension available only in \verb|clx|. The \verb|clx.Cplx| type constructor is parameterized by the numeric type that should be used for the real and imaginary parts, here \verb|clx.int|, the type corresponding to 32-bit OpenCL integers. Arithmetic is supported with other types in the same family, as well as with plain numeric types (treated as if their imaginary part was zero). When targeting OpenCL, Ace expressions of type \verb|clx.Cplx(clx.int)| are compiled to OpenCL expressions of type \verb|int2|, a pair of 32-bit integers. This can be observed in several places on lines \ref{mapout}.14-\ref{mapout}.21. This is merely an implementation detail of \verb|clx.Cplx|, however. An Ace value of type \verb|clx.int2| can\emph{not} be used when a \verb|clx.Cplx(clx.int)| is expected (attempting to do so will result in a static type error). That is, \verb|clx.Cplx| truly extends the type system, it is not merely a type alias (see Sec. \ref{atypes}).

The type \verb|TF| on line \ref{compscript}.5
